...
users.post('/register', async (req, res) => {
  ...
  if (!name || !email || !password) {
    return res.status(httpStatusCode.badRequest).json({ message: 'Invalid data' });
  }
  ...
  const payload = { issuer: 'post-api', audience: 'identity', userData: email };

  const token = await createJWT(payload);

  return res.status(httpStatusCode.created).json({ role, token });
});

users.put('/update', validateJWT, async (req, res) => {
  const { name, email } = req.body;

  if (!name || !email) return res.status(httpStatusCode.badRequest).
    json({ message: 'Invalid data' });

  await service.updateUser(name, email);

  return res.status(httpStatusCode.ok).json({ message: 'User updated successfully' });
});
...
users.get('/profile', validateJWT, async (req, res) => {
  ...
  return res.status(httpStatusCode.badRequest).json({ message: 'jwt malformed' });
});
