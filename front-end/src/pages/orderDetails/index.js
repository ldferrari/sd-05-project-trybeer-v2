...
function cartSumFunction(orderHere) {
  const [zero, dois] = [0, 2];
  return orderHere.reduce((acc, cv) => acc + cv.price * cv.quantity, zero).toFixed(dois);
}

function detailsHeaderFunction(globalData, id) {
  return (
...
function pedidoFunction(orderHere, cartSum) {
  return (
...
export default function OrderDetails(props) {
  ...
  return (
    <div className="orderDetailsPage">
      ...
      {detailsHeaderFunction(globalData, id)}
      {pedidoFunction(orderHere, cartSum)}
      <Footer />
    </div>
  );
}
